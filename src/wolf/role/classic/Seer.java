...
public class Seer extends GameRole {
  ...
  protected void onNightBegins() {
    ...
    getEngine().getBot().sendMessage(getPlayer(),
        "Tell me who you want to peek.  Message me '!peek [target]'");
  }

  public void addPeek(Player p) {
    peeks.put(p, p.getRole().onPeek());
  }
  ...
  protected void onNightEnds() {
    ...
    getEngine().getBot().sendMessage(getPlayer(),
        currentPeekTarget.getName() + " is a " + currentPeekTarget.getRole().onPeek());
  ...
  private final BotAction peekAction = new BotAction(1) {
    ...
    protected void execute(WolfBot bot, String sender, String command, List<String> args) {
      ...
      getEngine().getBot().sendMessage(sender,
          "Your peek has been received and will be revealed to you at the end of the night.");
    }
  };
